...
#define MsgTypeIsCtrl(type_and_size)        (!((type_and_size)&0xE0) && (type_and_size)&0x1C)
#else
#define MsgTypeIsCtrl(type_and_size)        (!((type_and_size)&0xE3) && (type_and_size)&0x1C)
#endif
#define MsgTypeIsData(type_and_size)        (!((type_and_size)&0x80) && (type_and_size)&0x60)
...
void MsgRecvHandler(void);
    static void MsgRecvIrqHandler(int ch);
...
void MsgWriteToDMA(unsigned char* buf, int len)
{
    ...
    if ( MsgSendIrqIsOver )
    {
        if ( (result=HAL_UART_Transmit_DMA(LCD_UART, buf, len)) == HAL_OK )
        {
        ...
        else
        {
            WI(MsgSendIrqIsOver);
            WI(LCD_UART->State);
            WATCHI("HAL_UART_Transmit_DMA() RETURN", result);
        }
    }
}
...
int MsgSend(int type, void* sendbuf, int bufsize)
{
    //DPRINTF(("MsgSend(%x,%i)", type, bufsize));
...
void MsgRecvHandler(void)
{
    ...
    while ( (ch = UARTgetc(LCD_UART)) >= 0 )
        MsgRecvIrqHandler(ch);
}
...
void MsgSendHandler(void)
{
	...
	int write_cnt;
    static volatile bool already_enter = false;
	static unsigned char write_buf[(DATA_MSG_MAX_LEN+1)*2+3];
    ...
    if ( MsgConnect )
    {
        ...
        if ( DataMsgTxQSendPos!=CQ_REAR(DataMsgTxQ) && already_tx_count<10 ) // 아직 ACK를 받지 못한 전송 메시지가 10개 이하인 경우
        {
    ...
    {
        // REQ_CONN을 반복해서 보낸다.
        if ( TICK_FROM(last_req_conn_tick)>=5000 ) // 5000ms
            MsgSend(MSG_TYPE_REQ_CONN, NULL, 0);
    }
